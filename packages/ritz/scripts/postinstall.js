...
const debug = require("debug")("ritz:postinstall")

const isInRitzMonorepo = fs.existsSync(path.join(__dirname, "../test"))
let isInstalledGlobally = isInRitzMonorepo ? false : true // default

try {
  const maybeGlobalRitzPath = resolveFrom(__dirname, "ritz")
  const localRitzPath = resolveFrom.silent(process.cwd(), "ritz")
  isInstalledGlobally = maybeGlobalRitzPath !== localRitzPath
} catch (error) {
  // noop
}
...
function codegen() {
  ...
  /**
   * Looks up for a `package.json` which is not `@ritz/cli` or `ritz` and returns the directory of the package
    ...
    for (let i = 0; i < limit; i++) {
      ...
      if (fs.existsSync(pkgPath)) {
        try {
          ...
          if (pkg.name && !["@ritz/cli", "ritz"].includes(pkg.name)) {
  ...
  async function main() {
    ...
    if (isUsingNpm && root && !fs.existsSync(path.join(root, "node_modules", "next"))) {
      ...
      // we use the `npm:@ritzjs/next` syntax to install the fork at node_modules/next
      debug("Missing next package, manually installing...")
      const corePkg = require("@ritzjs/core/package.json")
    ...
    debug({
      ...
      RITZ_GENERATE_IN_POSTINSTALL: process.env.RITZ_GENERATE_IN_POSTINSTALL,
    })
    try {
      if (localPath) {
        await run("node", [
      ...
      if (installedGlobally) {
        await run("ritz", ["codegen", "--postinstall", doubleQuote(getPostInstallTrigger())])
        return
      }
    } catch (e) {
    ...
    if (!localPath && !installedGlobally) {
      console.error(
        `In order to use "@ritz/client", please install Ritz CLI. You can install it with "npm add -D ritz".`,
      )
    }
  }
  ...
  function getLocalPackagePath() {
    try {
      const packagePath = require.resolve("ritz/package.json")
      if (packagePath) {
        return require.resolve("ritz")
      }
    } catch (e) {
      //
    }
    ...
    try {
      const packagePath = require.resolve("@ritz/cli/package.json")
      if (packagePath) {
        return require.resolve("@ritz/cli")
      }
    } catch (e) {}
  ...
  async function isInstalledGlobally() {
    try {
      await run("ritz", ["-v"], process.cwd(), ["ignore"])
      return true
    } catch (e) {
      return false
    }
  ...
  if (!process.env.RITZ_SKIP_POSTINSTALL_GENERATE) {
    main()
      .catch((e) => {
        console.error(e)
        process.exit(0)
  ...
  async function ensureEmptyDotRitz() {
    try {
      const dotRitzDir = isInRitzMonorepo
        ? path.join(process.cwd(), "node_modules/.ritz")
        : path.join(__dirname, "../../.ritz")

      await makeDir(dotRitzDir)
      const defaultIndexJsPath = path.join(dotRitzDir, "index.js")
      const defaultIndexBrowserJSPath = path.join(dotRitzDir, "index-browser.js")
      const defaultIndexDTSPath = path.join(dotRitzDir, "index.d.ts")
