...
import {getRitzDependencyVersion} from "../utils/get-ritz-dependency-version"
...
export class AppGenerator extends Generator<AppGeneratorOptions> {
  ...
  filesToIgnore() {
    if (!this.options.useTs) {
      return ["tsconfig.json", "ritz-env.d.ts", "jest.config.ts"]
    }
  ...
  async postWrite() {
    ...
    const [
      ...
      {value: ritzDependencyVersion, isFallback: ritzUsedFallback},
    ] = await Promise.all([
      fetchLatestVersionsFor(pkg.dependencies),
      fetchLatestVersionsFor(pkg.devDependencies),
      getRitzDependencyVersion(this.options.version),
    ])

    pkg.dependencies = newDependencies
    pkg.devDependencies = newDevDependencies
    pkg.dependencies.ritz = ritzDependencyVersion

    const fallbackUsed =
      dependenciesUsedFallback || devDependenciesUsedFallback || ritzUsedFallback
  ...
  commitChanges() {
    ...
    const commands: Array<[string, string[], object]> = [
      ...
      [
        ...
        [
          "-c",
          "user.name='Ritz.js CLI'",
          "-c",
          "user.email='noop@ritzjs.com'",
          "commit",
          "--no-gpg-sign",
          "--no-verify",
          "-m",
