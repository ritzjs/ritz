import {paths} from "@ritzjs/installer"
...
describe("path utils", () => {
  it("returns proper file paths in a TS project", () => {
    ...
    // Ritz and Babel configs are always JS, we shouldn't transform this extension
    expect(paths.ritzConfig()).toBe("ritz.config.ts")
  ...
  testIfNotWindows("returns proper file paths in a JS project", () => {
    ...
    expect(paths.ritzConfig()).toBe("ritz.config.js")
    expect(paths.babelConfig()).toBe("babel.config.js")
  })
})
