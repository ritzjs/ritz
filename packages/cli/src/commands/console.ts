...
export class Console extends Command {
  static description = "Run the Ritz console REPL"
  static aliases = ["c"]
  ...
  async run() {
    ...
    this.parse(Console)
    const {log} = require("@ritzjs/display")
    const chalk = require("chalk")
    log.branded("You have entered the Ritz console")
    ...
    require("@ritzjs/repl").runRepl(Console.replOptions)
  }
}
