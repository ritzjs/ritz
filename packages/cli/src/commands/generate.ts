...
import {
  ...
  singlePascal,
  uncapitalize,
} from "@ritzjs/generator"
import {flags} from "@oclif/command"
import chalk from "chalk"
import {Command} from "../command"
import {PromptAbortedError} from "../errors/prompt-aborted"

const debug = require("debug")("ritz:generate")
const getIsTypeScript = () =>
  require("fs").existsSync(
    require("path").join(require("@ritzjs/config").getProjectRoot(), "tsconfig.json"),
  )
...
export class Generate extends Command {
  static description = "Generate new files for your Ritz project"
  static aliases = ["g"]
  static strict = false
  static args = [
    {
  ...
  static flags = {
    ...
    context: flags.string({
      char: "c",
      description:
        "Provide a context folder within which we'll place the generated files for better code organization. You can also supply this in the name of the model to be generated (e.g. `ritz generate query admin/projects`). Combining the `--context` flags and supplying context via the model name in the same command is not supported.",
    }),
    parent: flags.string({
      char: "p",
      description:
        "Specify a parent model to be used for generating nested routes for dependent data when generating pages, or to create hierarchical validation in queries and mutations. The code will be generated with the nested data model in mind. Most often this should be used in conjunction with 'ritz generate all'",
    }),
...
> ritz generate all tasks --parent=projects
    `,
...
> ritz generate model task \\
    name:string \\
    completed:boolean:default=false \\
    belongsTo:project?
> ritz generate all tasks \\
    name:string \\
    completed:boolean:default=false \\
    belongsTo:project?
    `,
...
> ritz generate query getUserSession`,
  ]
  ...
  async handleNoContext(message: string): Promise<void> {
    ...
    if (!shouldCreateNewRoot) {
      require("@ritzjs/display").log.error(
