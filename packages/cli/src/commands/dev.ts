...
export class Dev extends Command {
  ...
  async run() {
    ...
    try {
      const dev = (await import("@ritzjs/server")).dev
      const {getConfig} = await import("@ritzjs/config")

      const ritzConfig = getConfig()
      if (
        ritzConfig.cli?.clearConsoleOnRitzDev !== false &&
        !process.env.RITZ_TEST_ENVIRONMENT
      ) {
        const {log} = await import("@ritzjs/display")
        log.clearConsole()
      }
