...
import type {RecipeExecutor} from "@ritzjs/installer"
import {flags} from "@oclif/command"
import {bootstrap} from "global-agent"
import {Stream} from "stream"
import {promisify} from "util"
import {Command} from "../command"
const debug = require("debug")("ritz:cli")
...
export class Install extends Command {
  static description = "Install a Recipe into your Ritz app"
  static aliases = ["i"]
  static strict = false
  ...
  static args = [
    {
      ...
      required: true,
      description:
        "Name of a Ritz recipe from @ritzjs/ritz/recipes, or a file path to a local recipe definition",
    },
    {
      name: "recipe-flags",
      description:
        "A list of flags to pass to the recipe. Ritz will only parse these in the form key=value",
    },
  ]
  ...
  async cloneRepo(
    ...
    const recipeDir = require("path").join(process.cwd(), ".ritz", "recipe-install")
  ...
  /**
   * Setup proxy support for ritz install
   *
   * Loads proxy variables from enviroment and ritz.config.js
   *
   */
...
1. The name of a recipe to install (e.g. "tailwind")
   ${chalk.dim(
     "- Available recipes listed at https://github.com/ritz-js/ritz/tree/main/recipes",
   )}
2. The full name of a GitHub repository (e.g. "ritz-js/example-recipe"),
3. A full URL to a Github repository (e.g. "https://github.com/ritz-js/example-recipe"), or
