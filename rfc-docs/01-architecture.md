# [RFC] Ritz App Architecture

**The purpose of this RFC is to gather as much feedback as possible before building everything outlined below.**

**We welcome all feedback, whether good or bad! This is your chance to ensure Ritz meets the needs for your company or project.**

<hr/>

## Introduction

The central thesis for Ritz is most apps don't need a REST or GraphQL API. Ritz brings back the simplicity of server rendered frameworks like Ruby on Rails while preserving everything we love about React.
...
[The Ritz Manifesto](https://github.com/ritz-js/ritz/blob/main/MANIFESTO.md) explains these principles in detail.
...
9. [Ritz is GraphQL Ready](#9-ritz-is-graphql-ready)
10. [SSR?](#10-ssr)
...
Ritz uses React and Next.js for the UI layer. Ritz provides a few conveniences on top of Next. But Ritz still gives you the raw power of Next.js to do anything you want. Including `getStaticProps`, `getServerSideProps`, `getInitialProps`, and custom API routes.

Under the hood, a Ritz app is compiled to a Next.js app for deployment. This gives us freedom to do things like use a totally different file structure that's better for fullstack apps.

## 3. Data Schema

Ritz is database agnostic. You are free to use anything at this layer, including something like TypeORM.
...
Validation is a key part of your data schema. Validation is needed both server-side and client-side. In a Ritz app, you define your validation rules one time, and then you can use them on both the server and the client.
...
Fine-grained authorization is another critical part of your data schema. In a Ritz app, you define your authorization rules one time, and then you can use them on both the server and the client.
...
So we've designed Ritz queries and mutations to be invoked via direct function calls.

### A. Queries

Define a Ritz query by exporting a plain Javascript function. The first function argument can be anything you want. The second argument will be provided by Ritz when executed from the frontend.
...
import {Context} from "ritz/types"
...
For public pages without private user data, you can use Ritz queries at build time for a fully static page.

```tsx
// pages/products/[id].tsx
import {PromiseReturnType} from "ritz/types"
import getProduct from "/some/path/getProduct"
...
import {useQuery, Router} from "ritz"
import getProduct from "/some/path/getProduct"
...
Ritz does some fancy stuff at compile time to convert the imported queries and mutations in your component into remote procedure calls (RPC). So your server code stays on the server and isn't actually included in your client-side bundle.
...
Queries and mutations are HTTP agnostic, but you still need a way to access raw HTTP for advanced use cases. For this, Ritz provides a middleware API that can add arbitrary data to the context object that's provided to the query/mutation function.

This example gets the `referer` and adds it to the context object.

```ts
// /some/path/special.ts
import {Context, ApiRequest, ApiResponse} from "ritz/types"
...
[Ritz file structure and routing are detailed in a separate RFC](https://github.com/ritz-js/ritz/pull/74). We'd love your feedback on that too!

## 6. Authentication

We are working on an authentication system that's highly secure and deeply integrated with Ritz. We will use Passport.js so you can use any of its strategies for identity verification. Then we are building an advanced solution for session management that has many features such as session timeout, session revocation, and anonymous session data that can be transferred to an authenticated session. Also it will automatically prevent against CSRF, XSS, and database session theft.

Ritz will automatically provide the authenticated session data to queries and mutations via the context argument.
...
## 9. Ritz is GraphQL Ready

Although Ritz doesn't use GraphQL, all your Ritz queries and mutations can easily be used as GraphQL resolvers.

## 10. SSR?

The initial Ritz announcement relied heavily on SSR. With the architecture in this RFC, SSR is no longer required (but SSR is still supported!).
...
Like Next.js, Ritz is agnostic as to your deployment type and host. Ritz apps are compiled to Next.js, so you can deploy a Ritz app in all the same ways you can deploy Next.js.

Ritz/Next are not tied to the Zeit platform. The build produces plain Javascript files you can run anywhere, including directly on AWS Lambda.

Also, Ritz/Next can be self-hosted on a traditional server, like a standard Express app, for example.

## 12. Background Processing

Asynchronous background processing is a very important for anything beyond trivial apps. If you host a Ritz app on a traditional server, you can do normal Node.js background processing with a library like Bull. But for serverless deployments, you need to use a third-party service like CloudAMQP.com.

We will be exploring ways to make this super easy for serverless environments.

## 13. Websockets?

Ritz currently has no plan for integrated websocket support because they aren't well supported in serverless environments and aren't currently supported by Prisma 2.

The recommended approach for live updates is to use polling. Polling is a first-class Ritz feature.
...
1. **Contributions of any kind, including code, design, documentation, and translation.**
   1. [Join the Ritz Discord group](https://discord.ritzjs.com)
