...
const nextTelemetry: cliCommand = (argv) => {
  ...
  if (args['--help']) {
    console.log(
      `
      Description
        Allows you to control Ritz.js' telemetry collection

      Usage
        $ next telemetry [enable/disable]

      You may pass the 'enable' or 'disable' argument to turn Ritz.js' telemetry collection on or off.
  ...
  if (args['--enable'] || args._[0] === 'enable') {
    ...
    if (isEnabled) {
      ...
      console.log(
        chalk.yellow(`Ritz.js' telemetry collection is already disabled.`)
      )
    }
    console.log()
    ...
    console.log(chalk.bold('Ritz.js Telemetry'))
    console.log()
  }
  ...
  if (isEnabled) {
    console.log(
      `Ritz.js telemetry is completely anonymous. Thank you for participating!`
    )
  } else {
    console.log(`You have opted-out of Ritz.js' anonymous telemetry program.`)
