...
const manifestDebug = require('debug')('ritz:manifest')
...
export async function saveRouteManifest(
  ...
  const dotRitz = join(await findNodeModulesRoot(directory), '.ritz')

  await outputFile(join(dotRitz, 'index.js'), implementation, {
    encoding: 'utf-8',
  })
  await outputFile(join(dotRitz, 'index-browser.js'), implementation, {
    encoding: 'utf-8',
  })
  await outputFile(join(dotRitz, 'index.d.ts'), declaration, {
    encoding: 'utf-8',
  })
}
...
async function findNodeModulesRoot(src: string) {
   ...
   *  we must first find ritz package, and then find @ritzjs/core and then
   *  the root of @ritzjs/core
   *
   *  This is because we import from `.ritz` inside @ritzjs/core.
  ...
  if (process.env.NEXT_PNPM_TEST) {
    ...
    if (!nextPkgLocation) {
      throw new Error(
        "Internal Ritz Error: unable to find 'next' package location"
      )
    }
    ...
    const ritzPkgLocation = dirname(
      (await findUp('package.json', {
        cwd: resolveFrom(src, 'ritz'),
      })) ?? ''
    )
    manifestDebug('ritzPkgLocation ' + ritzPkgLocation)
    if (!ritzPkgLocation) {
      throw new Error(
        "Internal Ritz Error: unable to find 'ritz' package location"
      )
    }
    const ritzCorePkgLocation = dirname(
      (await findUp('package.json', {
        cwd: resolveFrom(ritzPkgLocation, '@ritzjs/core'),
      })) ?? ''
    )
    manifestDebug('ritzCorePkgLocation ' + ritzCorePkgLocation)
    if (!ritzCorePkgLocation) {
      throw new Error(
        "Internal Ritz Error: unable to find '@ritzjs/core' package location"
      )
    }
    root = join(ritzCorePkgLocation, '../../')
  }
...
export function generateManifest(
  ...
  const moduleName = process.env.NEXT_PNPM_TEST ? 'next/types' : 'ritz'
