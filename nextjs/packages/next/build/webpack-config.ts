...
export default async function getBaseWebpackConfig(
  ...
  const hasReactExperimental: boolean =
    Boolean(reactDomVersion) && reactDomVersion!.includes('experimental') // ritz
  ...
  let webpackConfig: webpack.Configuration = {
    ...
    entry: async () => {
      return {
        ...
        ...(isServer && hasDbModule ? { 'ritz-db': './db/index' } : {}),
        ...entrypoints,
      }
    },
    watchOptions: {
      ...
      new webpack.DefinePlugin({
        ...
        ...Object.keys(config.env).reduce((acc, key) => {
          if (/^(?:NODE_.+)|^(?:__.+)$/i.test(key)) {
            throw new Error(
              `The key "${key}" under "env" in ritz.config.js is not allowed. https://nextjs.org/docs/messages/env-key-not-allowed`
            )
          }
        ...
        ...(dev && !isServer
          ...
          : {}),
        'process.env.__RITZ_SUSPENSE_ENABLED': JSON.stringify(hasReactRoot),
        'process.env.__RITZ_SESSION_COOKIE_PREFIX': JSON.stringify(
          sessionCookiePrefix
        ),
  ...
  if (isWebpack5) {
    ...
    const cache: any = {
      ...
      // Includes:
      //  - Next.js version
      //  - ritz.config.js keys that affect compilation
      version: `${process.env.__NEXT_VERSION}|${configVars}`,
      cacheDirectory: path.join(distDir, 'cache', 'webpack'),
    }

    // TODO - can we remove this?
    // Adds `ritz.config.js` as a buildDependency when custom webpack config is provided
  ...
  if (typeof config.webpack === 'function') {
    ...
    if (!webpackConfig) {
      throw new Error(
        'Webpack config is undefined. You may have forgot to return properly from within the "webpack" method of your ritz.config.js.\n' +
    ...
    if (typeof (webpackConfig as any).then === 'function') {
      console.warn(
        '> Promise returned in ritz config. https://nextjs.org/docs/messages/promise-in-next-config'
      )
    }
  }
  ...
  if (
    ...
    if (foundTsRule) {
      console.warn(
        '\n@zeit/next-typescript is no longer needed since Ritz.js has built-in support for TypeScript now. Please remove it from your ritz.config.js and your .babelrc\n'
      )
    }
  }
