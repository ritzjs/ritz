...
function getPlugins(
  ...
  const rpcClientConfigItem =
    !isServer && isRpcFile
      ? createConfigItem([require('../plugins/ritz-rpc-client')], {
          type: 'plugin',
        })
      : null
  const rpcServerTransformConfigItem =
    isServer && isRpcFile
      ? createConfigItem([require('../plugins/ritz-rpc-server-transform')], {
          type: 'plugin',
        })
      : null
...
function getCustomBabelConfig(configFilePath: string) {
  ...
  throw new Error(
    'The Ritz.js Babel loader does not support .mjs or .cjs config files.'
  )
}
