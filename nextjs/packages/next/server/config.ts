...
function assignDefaults(userConfig: { [key: string]: any }) {
  if (typeof userConfig.exportTrailingSlash !== 'undefined') {
    console.warn(
      chalk.yellow.bold('Warning: ') +
        'The "exportTrailingSlash" option has been renamed to "trailingSlash". Please update your ritz.config.js.'
    )
  ...
  if (typeof userConfig.experimental?.reactMode !== 'undefined') {
    console.warn(
      chalk.yellow.bold('Warning: ') +
        'The experimental "reactMode" option has been replaced with "reactRoot". Please update your ritz.config.js.'
    )
    ...
    (currentConfig, key) => {
      ...
      if (key === 'distDir') {
        ...
        if (userDistDir === 'public') {
          throw new Error(
            `The 'public' directory is reserved in Ritz.js and can not be set as the 'distDir'. https://nextjs.org/docs/messages/can-not-output-to-public`
          )
        }
...
export default async function loadConfig(
  ...
  if (!['start', 's'].includes(process.argv[2])) {
    // Do not compile config for ritz start because it was already compiled during ritz build
  ...
  if (path?.length) {
    ...
    if (Object.keys(userConfig).length === 0) {
      Log.warn(
        'Detected ritz.config.js, no exported configuration found. https://nextjs.org/docs/messages/empty-configuration'
      )
    }
    ...
    if (unsupportedPath?.length) {
      throw new Error(
        `Configuring Ritz.js via '${basename(
          unsupportedPath
        )}' is not supported. Please replace the file with 'ritz.config.(js|ts)'`
      )
    }
  }
