...
export function loadConfigAtRuntime() {
  ...
  return loadConfigProduction(process.env.RITZ_APP_DIR)
}
...
export async function getConfigSrcPath(dir: string | null) {
  ...
  let tsPath = join(dir, 'ritz.config.ts')
  let jsPath = join(dir, 'ritz.config.js')
  ...
  if (existsSync(tsPath)) {
    ...
    if (isInternalDevelopment || process.env.VERCEL_BUILDER) {
      ...
      console.log('') // newline
      throw new Error(
        'Ritz does not support next.config.js. Please rename it to ritz.config.js'
      )
    }
  }

  if (process.env.__NEXT_TEST_MODE) {
    let tsPath2 = join(dir, '..', 'ritz.config.ts')
    let jsPath2 = join(dir, '..', 'ritz.config.js')
...
export async function compileConfig(dir: string | null) {
  ...
  if (!pkgJsonPath) {
    // This will happen when running ritz no inside a ritz app
  ...
  await esbuild({
    ...
    external: [
      ...
      'next',
      'webpack',
      ...Object.keys(require('ritz/package').dependencies),
