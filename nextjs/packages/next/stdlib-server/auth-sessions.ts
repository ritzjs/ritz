// Must import this type from 'ritz'
...
const debug = require('debug')('ritz:session')
...
const getDb = () => {
  if (!process.env.RITZ_APP_DIR) {
    throw new Error(
      'Internal Ritz Error: process.env.RITZ_APP_DIR is not set'
    )
  }
  const projectRoot = process.env.RITZ_APP_DIR
  let path = join(projectRoot, '.next/server/ritz-db.js')
  if (!fs.existsSync(path)) {
    path = join(projectRoot, '.next/serverless/ritz-db.js')
  }
...
export const sessionMiddleware = (
  ...
  const cookiePrefix = global.sessionConfig.cookiePrefix ?? 'ritz'
  assert(
  ...
  const ritzSessionMiddleware: Middleware = async (req, res, next) => {
    debug('Starting sessionMiddleware...')
    if (!(res.ritzCtx as any).session) {
      // This function also saves session to res.ritzCtx
      await getSession(req, res)
    }
    return next()
  }

  ritzSessionMiddleware.config = {
    name: 'ritzSessionMiddleware',
    cookiePrefix,
  }
  return ritzSessionMiddleware
}
...
export async function getSession(
  ...
  debug('cookiePrefix', process.env.__RITZ_SESSION_COOKIE_PREFIX)

  let response = res as MiddlewareResponse<{ session?: SessionContext }>

  if (response.ritzCtx.session) {
    return response.ritzCtx.session
  }
  ...
  response.ritzCtx.session = sessionContext
...
export class SessionContextClass implements SessionContext {
  ...
  $isAuthorized(...args: IsAuthorizedArgs) {
    ...
    return global.sessionConfig.isAuthorized({ ctx: this._res.ritzCtx, args })
  }
...
const JWT_NAMESPACE = 'ritzjs'
const JWT_ISSUER = 'ritzjs'
const JWT_AUDIENCE = 'ritzjs'
  ...
  if (sessionToken) {
    ...
    if (
      ...
      if (!antiCSRFToken) {
        // log.warning(
        console.log(
          `This request is missing the ${HEADER_CSRF} header. You can learn about adding this here: https://ritzjs.com/docs/session-management#manual-api-requests`
        )
      }
