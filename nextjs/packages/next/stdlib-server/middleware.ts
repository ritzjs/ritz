...
>(
  ...
  middleware.push(async (_req, res, next) => {
    ...
    try {
      ...
      const result = await interopDefault(rpcResolver)(params, res.ritzCtx)

      const duration = Date.now() - startTime
      log.info(chalk.dim(`Finished in ${prettyMs(duration)}`))
      newline()

      res.ritzResult = result
      return next()
    } catch (error) {
      throw error
    }
  ...
  return (ctx.res as MiddlewareResponse).ritzResult as TResult
}
...
/**
 * Returns a Ritz middleware function that varies its async logic based on if the
