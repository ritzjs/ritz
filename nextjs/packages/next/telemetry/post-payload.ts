...
export function _postPayload(data: Data) {
  ...
  const { nextVersion: ritzVersion, ...meta } = data.meta
  ...
  return (
    retry(
      () =>
        fetch(
          `https://us-east-1-1.aws.cloud2.influxdata.com/api/v2/write?org=ritz&bucket=ritzjs`,
          {
            method: 'POST',
            body: lines,
            headers: {
