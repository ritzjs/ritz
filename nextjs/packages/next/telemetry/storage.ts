...
export class Telemetry {
  ...
  constructor({ distDir }: { distDir: string }) {
    ...
    const {
      ...
      RITZ_TELEMETRY_DISABLED,
      RITZ_TELEMETRY_DEBUG,
    } = process.env

    this.NEXT_TELEMETRY_DISABLED =
      RITZ_TELEMETRY_DISABLED || NEXT_TELEMETRY_DISABLED
    this.NEXT_TELEMETRY_DEBUG = RITZ_TELEMETRY_DEBUG || NEXT_TELEMETRY_DEBUG
  ...
  private notify = () => {
    ...
    console.log(
      `${chalk.magenta.bold(
        'Attention'
      )}: Ritz.js now collects completely anonymous telemetry regarding usage.`
    )
    console.log(
      `This information is used to shape Ritz.js' roadmap and prioritize features.`
    )
    console.log(
