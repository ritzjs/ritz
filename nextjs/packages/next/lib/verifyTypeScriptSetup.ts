...
export async function verifyTypeScriptSetup(
  ...
  try {
    ...
    if (
      ...
      !Boolean(process.env.RITZ_TEST_ENVIRONMENT)
    ) {
      log.warn(
        `Minimum recommended TypeScript version is v4.3.2, older versions can potentially be incompatible with Ritz.js. Detected: ${ts.version}`
      )
    }
