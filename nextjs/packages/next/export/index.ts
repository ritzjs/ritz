...
export default async function exportApp(
  ...
  return nextExportSpan.traceAsyncFn(async () => {
    ...
    // attempt to load global env values so they are available in ritz.config.js
    nextExportSpan
    ...
    if (!existsSync(buildIdFile)) {
      throw new Error(
        `Could not find a production build in the '${distDir}' directory. Try building your app with 'ritz build' before starting the static export. https://nextjs.org/docs/messages/next-export-no-build-id`
      )
    }
    ...
    if (outDir === join(dir, 'public')) {
      throw new Error(
        `The 'public' directory is reserved in Ritz.js and can not be used as the export out directory. https://nextjs.org/docs/messages/can-not-output-to-public`
      )
    }

    if (outDir === join(dir, 'static')) {
      throw new Error(
        `The 'static' directory is reserved in Ritz.js and can not be used as the export out directory. https://nextjs.org/docs/messages/can-not-output-to-static`
      )
    }
    ...
    if (typeof nextConfig.exportPathMap !== 'function') {
      if (!options.silent) {
        Log.info(
          `No "exportPathMap" found in "ritz.config.js". Generating map from "./pages"`
        )
      }
    ...
    if (i18n && !options.buildExport) {
      throw new Error(
        `i18n support is not compatible with ritz export. See here for more info on deploying: https://nextjs.org/docs/deployment`
      )
    }
    ...
    if (!options.buildExport) {
      ...
      if (middleware?.length) {
        throw new Error(
          `Ritz http middleware is not compatible with \`ritz export\`.
  Possible solutions:
    - Use \`ritz start\` to run a server, which supports middleware.
    - Remove \`middleware\` from \`ritz.config.js\`.\n`
        )
      }
  ...
  Possible solutions:
    - Use \`ritz start\` to run a server, which includes the Image Optimization API.
    - Use any provider which supports Image Optimization (like Vercel).
    - Configure a third-party loader in \`ritz.config.js\`.
    - Use the \`loader\` prop for \`ritz\`.
  Read more: https://nextjs.org/docs/messages/export-image-api`
    ...
    if (hasApiRoutes) {
      if (!options.silent) {
        Log.warn(
          chalk.yellow(
            `Statically exporting a Ritz.js application via \`ritz export\` disables API routes.`
          ) +
            `\n` +
            chalk.yellow(
            ...
            `\n` +
            chalk.yellow(
              `Pages in your application without server-side data dependencies will be automatically statically exported by \`ritz build\`, including pages powered by \`getStaticProps\`.`
            ) +
            `\n` +
            chalk.yellow(
