...
;['react', 'react-dom'].forEach((dependency) => {
  try {
    ...
    console.warn(
      `The module '${dependency}' was not found. Ritz.js requires that you include it in 'dependencies' of your 'package.json'. To add it, run 'npm install ${dependency}'`
    )
  }
})
...
if (args['--version']) {
  console.log(`Ritz.js v${process.env.__NEXT_VERSION}`)
  process.exit(0)
}
...
if (command === 'dev') {
  ...
  watchFile(`${process.cwd()}/ritz.config.js`, (cur: any, prev: any) => {
    if (cur.size > 0 || prev.size > 0) {
      console.log(
        `\n> Found a change in ritz.config.js. Restart the server to see the changes in effect.`
      )
    }
  })
  watchFile(`${process.cwd()}/ritz.config.ts`, (cur: any, prev: any) => {
    if (cur.size > 0 || prev.size > 0) {
      console.log(
        `\n> Found a change in ritz.config.ts. Restart the server to see the changes in effect.`
      )
    }
  })
}
