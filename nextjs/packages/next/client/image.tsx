...
function defaultImageLoader(loaderProps: ImageLoaderProps) {
  ...
  throw new Error(
    `Unknown "loader" found in "ritz.config.js". Expected: ${VALID_LOADERS.join(
      ', '
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    if (!src.startsWith('/') && configDomains) {
      ...
      if (
        ...
        throw new Error(
          `Invalid src prop (${src}) on \`<Image>\`, hostname "${parsedSrc.hostname}" is not configured under images in your \`ritz.config.js\`\n` +
