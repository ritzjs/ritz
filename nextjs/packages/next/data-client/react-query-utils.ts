...
export const initializeQueryClient = () => {
  ...
  if (!process.env.CLI_COMMAND_CONSOLE && !process.env.CLI_COMMAND_DB) {
    suspenseEnabled = Boolean(process.env.__RITZ_SUSPENSE_ENABLED)
  }
...
const isNotInUserTestEnvironment = () => {
  ...
  if (process.env.RITZ_TEST_ENVIRONMENT !== undefined) return true
  return false
}
...
export const validateQueryFn = <TInput, TResult>(
  ...
  if (isClient && !isRpcClient(queryFn) && isNotInUserTestEnvironment()) {
    throw new Error(
      `Either the file path to your resolver is incorrect (must be in a "queries" or "mutations" folder that isn't nested inside "pages" or "api") or you are trying to use Ritz's useQuery to fetch from third-party APIs (to do that, import useQuery directly from "react-query")`
    )
  }
}
...
export function setQueryData<TInput, TResult, T extends AsyncFunc>(
  ...
  return new Promise((res) => {
    ...
    if (isClient) {
      // Fix for https://github.com/ritz-js/ritz/issues/1174
      requestIdleCallback(() => {
        res(result)
      })
    } else {
