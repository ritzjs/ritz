...
const prefix = () => {
  if (!process.env.__RITZ_SESSION_COOKIE_PREFIX) {
    throw new Error(
      'Internal Ritz Error: process.env.__RITZ_SESSION_COOKIE_PREFIX is not set'
    )
  }
  return process.env.__RITZ_SESSION_COOKIE_PREFIX
}

export const COOKIE_ANONYMOUS_SESSION_TOKEN = () =>
  `${prefix()}_sAnonymousSessionToken`
...
export const LOCALSTORAGE_PREFIX = '_ritz-'
