...
import {
  ...
  buildQueryRpc,
  buildMutationRpc,
} from '../ritz-test-utils'

beforeAll(() => {
  queryClient.clear()
  process.env.__RITZ_SESSION_COOKIE_PREFIX = 'ritz-test'
  process.env.__RITZ_SUSPENSE_ENABLED = 'true'
})
afterAll(() => {
  jest.clearAllMocks()
  process.env.__RITZ_SESSION_COOKIE_PREFIX = undefined
  process.env.__RITZ_SUSPENSE_ENABLED = undefined
})
...
describe('useQuery', () => {
  ...
  describe('a "query" that converts the string parameter to uppercase', () => {
    ...
    it('should work with Ritz queries', async () => {
