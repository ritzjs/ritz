...
export function initRitzServerScript(
  scriptPath: string,
...
interface RunRitzCommandOptions {
  cwd?: string
  env?: Record<any, any>
  spawnOptions?: any
  instance?: any
...
export function runRitzCommand(argv: any[], options: RunRitzCommandOptions = {}) {
  const ritzDir = path.dirname(require.resolve("ritz/package"))
  const ritzBin = path.join(ritzDir, "bin/ritz")
  const cwd = options.cwd || ritzDir
  ...
  return new Promise<any>((resolve, reject) => {
    console.log(`Running command "ritz ${argv.join(" ")}"`)
    const instance = spawn("node", ["--no-deprecation", ritzBin, ...argv], {
...
interface RunRitzLaunchOptions {
  cwd?: string
  env?: Record<any, any>
  onStdout?: (stdout: string) => void
  onStderr?: (stderr: string) => void
...
export function runRitzLaunchCommand(
  argv: any[],
  stdOut: unknown,
  opts: RunRitzLaunchOptions = {},
) {
  const ritzDir = path.dirname(require.resolve("ritz/package"))
  const ritzBin = path.join(ritzDir, "bin/ritz")
  const cwd = opts.cwd ?? path.dirname(require.resolve("ritz/package"))
  console.log(cwd)
  const env = {
    ...process.env,
    NODE_ENV: undefined,
  ...
  const command = opts.ritzStart ? "start" : "dev"
  console.log(`Running command "ritz ${command}" `)

  return new Promise<void | string | ChildProcess>((resolve, reject) => {
    const instance = spawn("node", ["--no-deprecation", ritzBin, command, ...argv], {cwd, env})
    ...
    function handleStdout(data: Buffer) {
      ...
      if (bootupMarkers[opts.ritzStart || stdOut ? "start" : "dev"].test(message)) {
        if (!didResolve) {
          didResolve = true
...
export function launchApp(dir: string, port: number, opts: RunRitzLaunchOptions = {}) {
  return runRitzLaunchCommand(["-p", port], undefined, {cwd: dir, ...opts})
}

export function ritzBuild(dir: string, args = [], opts: RunRitzCommandOptions = {}) {
  return runRitzCommand(["build", ...args], {cwd: dir, ...opts})
}

export function ritzExport(dir: string, {outdir}, opts: RunRitzCommandOptions = {}) {
  return runRitzCommand(["export", "--outdir", outdir], {cwd: dir, ...opts})
}

export function ritzExportDefault(dir: string, opts: RunRitzCommandOptions = {}) {
  return runRitzCommand(["export"], {cwd: dir, ...opts})
}

export function ritzStart(dir: string, port: number, opts: RunRitzLaunchOptions = {}) {
  return runRitzLaunchCommand(["-p", port], undefined, {
    cwd: dir,
    ...opts,
    ritzStart: true,
  })
...
export function buildTS(args = [], cwd: string, env = {}) {
  cwd = cwd || path.dirname(require.resolve("@ritzjs/cli/package"))
...
export function readRitzBuildClientPageFile(appDir: string, page: string) {
...
export function readRitzBuildServerPageFile(appDir: string, page: string) {
