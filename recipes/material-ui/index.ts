...
This will let the next.js app opt out of the React.Strict mode wrapping. Once you switch to legacy mode, you will also have to pass { suspense: false } to the useQuery options when querying data endpoints in your pages/components. You can check the documentation for useQuery at https://ritzjs.com/docs/use-query#options`,
  )
  ...
  .addTransformFilesStep({
    ...
    transform(program: Collection<j.Program>) {
      ...
      program.find(j.ImportDeclaration, {source: {value: "ritz"}}).forEach((ritzImportPath) => {
        if (
          !ritzImportPath.value.specifiers
            .filter((spec) => j.ImportSpecifier.check(spec))
            .some((node) => (node as j.ImportSpecifier)?.imported?.name === "DocumentContext")
        ) {
          ritzImportPath.value.specifiers.splice(
            0,
            0,
