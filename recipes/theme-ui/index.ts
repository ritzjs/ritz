import {addImport, paths, RecipeBuilder} from "@ritzjs/installer"
...
function wrapRitzConfigWithNextMdxPlugin(program: Collection<j.Program>) {
  program
    .find(j.Identifier, {
      name: "module",
    })
...
// Copied from https://github.com/ritz-js/ritz/pull/805, let's add this to the @ritzjs/installer
...
function updateRitzConfigProperty(program: Collection<j.Program>, property: string) {
  return program
...
export default RecipeBuilder()
  ...
  .addTransformFilesStep({
    stepId: "createOrModifyRitzConfig",
    stepName: "Add the '@next/mdx' plugin to the ritz config file",
    explanation: `Now we have to update our ritz config to support MDX`,
    singleFileSearch: paths.ritzConfig(),
    ...
    transform(program: Collection<j.Program>) {
      ...
      updateRitzConfigProperty(program, "pageExtensions")
      return wrapRitzConfigWithNextMdxPlugin(program)
    },
  })
  .addTransformFilesStep({
